...
# RFID variables
rfid_port = 'COM3'  # Change this to your RFID reader's serial port
rfid_baudrate = 9600
rfid_reader = None
rfid_thread = None
rfid_tag = None
...
def rfid_reader_thread():
    global rfid_tag
    try:
        rfid_reader = serial.Serial(rfid_port, rfid_baudrate, timeout=1)
        while running:
...
while running:
    ...
    if current_state == MENU:
        ...
        if not show_feedback and rfid_answer is not None:
        ...
        if not show_feedback and mouse_click:
